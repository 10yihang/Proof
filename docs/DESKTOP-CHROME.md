...
仓库、Branch、页面和 Diff tabs 共用一条 48px 标题栏；窄窗口优先缩短仓库名称和次要按钮，tabs 在自己的区域横向滚动。macOS 红黄绿仍预留 88px，交互控件不作为拖动区域。

文件行 32px、目录层级缩进 20px，默认文件栏由 240px 调整为 320px，已保存的用户宽度继续保留。长文件名可横向滚动；文件操作和勾选入口在 hover / focus 时显示。搜索和 Tree / List 切换合并为一行，批量操作栏固定在文件树底部，勾选和清除不改变目录位置。

Commit 左栏上方选择 Stage / Unstage 文件，下方输入说明与 Amend；右侧共用 Local changes 的同一个 Diff 实例。切页保留文件、阅读状态和提交草稿。
...
| 参考交互 | Proof 的处理 |
| --- | --- |
| 仓库操作、导航和内容各有固定位置 | 仓库与页面、比较 tabs 共用 48px 标题栏，tabs 自身可横向滚动 |
| 清晰的文件层级与连续提交线 | Commit 与文件行均为 32px，文件夹缩进 20px，虚拟列表坐标与行高同步 |
